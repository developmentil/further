...
Server.prototype._start = function(callback) {
	...
	this.server.listen(this._port, this._host, this._backlog, function(err){
		...
		further.logger.log('info', self._serviceName + ' listening on port %d', 
	...
	further.logger.log('info', this._serviceName + ' started on port %d', this._port);
};
...
Server.prototype._stop = function(callback) {
	...
	this.server.close(function(err) {
		if(!err) {
			further.logger.log('info', self._serviceName + ' stopped on port %d', self._port);
		}
