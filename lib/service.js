...
Service.prototype.start = function(callback) {
	...
	this.onIdle(function() {
		if(self._serviceName)
			further.logger.log('info', self._serviceName + ': Starting Service...');
		...
		var myCallback = self.setBusy(STATUS_STARTING, function(err) {
			...
			if(self._serviceName)
				further.logger.log('info', self._serviceName + ': Service Started.');
...
Service.prototype.stop = function(callback) {
	...
	this.onIdle(function() {
		if(self._serviceName)
			further.logger.log('info', self._serviceName + ': Stopping Service...');
		...
		var myCallback = self.setBusy(STATUS_STOPING, function(err) {
			...
			if(self._serviceName)
				further.logger.log('info', self._serviceName + ': Service Stopped.');
...
Service.prototype.setBusy = function(status, callback) {
	...
	myCallback = function() {
		...
		debug(self._serviceName + ': Service set as idle (' + self._status + ')');
	...
	debug(this._serviceName + ': Service set as busy (' + this._status + ')');
	this.emit('busy');
